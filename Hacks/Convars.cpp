...
void Hacks::find_and_set_convars() {
	...
	vmt.cvar->g_convars["dota_camera_distance"]->set<float>(static_cast<float>(ConfigManager::camera_distance));
	vmt.cvar->g_convars["r_farz"]->set<float>(static_cast<float>(ConfigManager::camera_distance * 2));
